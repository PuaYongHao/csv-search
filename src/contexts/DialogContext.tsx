...
const DialogContextProvider = ({ children }: DialogContextProviderProps) => {
  ...
  const [dialogState, setDialogState] =
    React.useState<DialogState>(initDialogState);

  const handleOpenDialog = React.useCallback(
    (stateEnum: DialogStateEnum) =>
      setDialogState({ ...initDialogState, [stateEnum]: true }),
    []
  );
  ...
  return (
    <DialogContext.Provider
      value={{ dialogState, handleCloseDialog, handleOpenDialog }}
    >
      {children}
    </DialogContext.Provider>
  );
